...
int main(void)
 {int n,pos ;
	...
	struct trabajador x[MAX] ;
 	...
 	cout<<"Ingresa el numero de trabajadores:\n" ;cin>>n ;
 	leerpersonal(n,x) ;
 	ordenaxfecha(n,x) ;
 	listado(n,x) ;
 	cout<<"Ingresa la fecha a buscar: \n"<<endl ;
 	cout<<"Ingresa el dia: \n" ;cin>>f.dia ;
 	cout<<"Ingresa el mes: \n" ;cin>>f.mes ;
 	cout<<"Ingresa el mes: \n" ;cin>>f.anio ;
 	pos=bBinariafecha(n,x,f) ;
 		if(pos==-1)cout<<"Trabajador no encontrado";
 		else cout<<"Trabajador numero \n"<<pos;
...
void listado(int n,struct trabajador x[]){
	...
	<<"/"<<x[i].nacimiento.mes<<"/"<<x[i].nacimiento.anio<<endl;
	cout<<endl;
}
